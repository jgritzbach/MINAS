class FormularOddluzeniManager{
    ...
    vyhodnotKolonky(){
        ...
        if (f._jeNevyplnene(f.kolonkaPlneMoci)){
            return `Není vyplněna kolonka náležitostí plné moci. Plnou  moc je přitom třeba zkoumat přednostně před ostatními kolonkami, které máte vyplněné. Ukázala-li by se plná moc jako vadná, je vyhodnocování ostatních náležitostí předčasné a tudíž zbytečné.`
        }
        ...
        if (f._jeVadne(f.kolonkaFormaPodani)){
            return `Forma podání insolvenčního návrhu spojeného s návrhem na povolení oddlužení je vadná. Zpracovatelem návrhu na povolení oddlužení je typicky advokát, insolvenční správce nebo akreditovaná osoba. Jedná se o osoby, které mají zřízenou datovou schránku ze zákona a vůči insolvenčnímu soudu tak mohou činit podání pouze v elektronické podobě (srov. § 80a IZ).<br/><br/>
        ...
        if (f._jeVadne(f.kolonkaMistniPrislusnost)){
            return `Místní příslušnost Krajského soudu patří mezi podmínky řízení, bez kterých není možné rozhodnout ve věci samé (srov. § 7b IZ). Sezná-li soud, že je místně nepříslušný, postoupí věc místně příslušnému soudu.<br/><br/>
        ...
        if (f._jeNevyplnene(f.kolonkaMistniPrislusnost)){
            return `Není vyplněna kolonka místní příslušnosti. Místní příslušnost je přitom třeba zkoumat přednostně před následnými kolonkami, které máte vyplněné. Ukázalo-li by se, že soud není místně příslušný, je vyhodnocování následných náležitostí předčasné a tudíž zbytečné.`
        }
        ...
        if (f._jeNevyplneneNecoZ(f.prilohyInsolvencnihoNavrhu)){
            return `Nejsou vyplněny všechny kolonky příloh insolvenčního návrhu. Přílohy insolvenčního návrhu je přitom třeba zkoumat přednostně před následnými kolonkami, které máte vyplněné. Pokud by vady příloh insolvenčního návrhu  nebyly odstraněny, insolvenční návrh by byl odmítnut, a vyhodnocování příloh návrhu na povolení oddlužení by bylo předčasné a tudíž zbytečné.`
        }
        ...
        if (f._jeDiskutabilniNecoZ(f.vsechnyKolonky)){
            return `Všechny kolonky byly vyplněny, ale některé náležitosti jsou diskutabilní. Insolvenční návrh spojený s návrhem na povolení oddlužení by mohl u soud obstát, ale konkrétní výsledek závisí na okolnostech případu.`
        }

        // nenastala-li žádná z přednostních podmínek, je vše v pořádku
        return `Všechny náležitosti insolvenčního návrhu spojeného s návrhem na povolení oddlužení se zdají být v pořádku. Soud by mohl rozhodnout o úpadku dlužníka a povolit jeho oddlužení.`
